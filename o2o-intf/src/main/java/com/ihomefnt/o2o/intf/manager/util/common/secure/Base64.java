...
public class Base64 implements BinaryDecoder {
    ...
    public static byte[] encodeBase64(byte[] binaryData, boolean isChunked) {
        ...
        if (isChunked) {
            nbrChunks = CHUNK_SEPARATOR.length == 0 ? 0 : (int)Math.ceil((double)encodedDataLength / 76.0);
    ...
    static byte[] discardWhitespace(byte[] data) {
        ...
        while(i < data.length) {
            if (data[i] != 9 && data[i] != 10 && data[i] != 13 && data[i] != 32) {
